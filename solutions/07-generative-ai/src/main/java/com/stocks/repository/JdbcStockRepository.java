...
public class JdbcStockRepository implements StockRepository {
    ...
    public Stock save(Stock stock) {
        ...
        return new Stock(keyHolder.getKey().longValue(),
                stock.symbol(), stock.companyName(), stock.sector(), stock.exchange());
    }

    @Override
    public List<Stock> findByCompanyNameContaining(String query) {
        return jdbc.query(
                "SELECT * FROM stock WHERE LOWER(company_name) LIKE LOWER(?) ORDER BY symbol",
                stockRowMapper,
                "%" + query + "%"
        );
    }
